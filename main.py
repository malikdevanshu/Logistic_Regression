...
from src.preprocessing import scale_features, splitting, validate_data
...
def main():
    ...
    for name, model in sklearn_models.items():
        cv = run_cross_validation(
            model, X_train_sc, y_train_np, cv=CV_FOLDS, scoring="accuracy"
        )
        cv_results[name] = cv
        fold_scores = "  ".join(f"{s:.3f}" for s in cv["scores"])
        print(
            f"  {name:<14} {cv['mean']:>10.4f} {cv['std']:>8.4f}  [{fold_scores}]"
        )
